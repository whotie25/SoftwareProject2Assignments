...
void manipulateServo(boolean _isOpen) {
  ...
  while(true) {
    ...
    if(elapsed_time <= MOVING_TIME) {
      int mapped_time = map(elapsed_time, 0, MOVING_TIME, 0, 10000);
      float quadratic = pow(mapped_time/10000.0 - manipulator, 2)*10000;
      int angle = map(quadratic, 0, 10000, startAngle, stopAngle);
