...
float dist_raw, dist_filtered, dist_ema, dist_median;

const int N = 30;             // array size
float rawDataArr[N] = {.0f};  // data array
int idx = 0;
...
void loop() {
  ...
  dist_median = medianFilter(rawDataArr);
...
float medianFilter(float* arr){
  ...
  else return 0.5f*(copiedArr[N/2 - 1] + copiedArr[N/2]);
}
